...
public class remoteUserAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    public static final int SECTION_VIEW = 0;
    public static final int CONTENT_VIEW = 1;

    private SearchContract.Presenter mPresenter;

    public remoteUserAdapter(SearchContract.Presenter presenter) {
        super();
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (SECTION_VIEW == getItemViewType(position)) {
            ...
            holder.itemView.setOnClickListener(view -> {
                mPresenter.addFavoriteUser(mPresenter.getUserItem(position));
            });
